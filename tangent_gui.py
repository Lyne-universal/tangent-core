...
class TangentApp(ctk.CTk):
    def __init__(self):
        ...
        # Initialize the Tangent logic core
        ...
        self.title_label = ctk.CTkLabel(self.header, text="TANGENT SECURE SCRUBBER", font=ctk.CTkFont(size=18, weight="bold"))
        ...
        # Runtime Cache Variables (Typed explicitly to ease Pylance constraints)
    ...
    def run_engine_analysis(self):
        ...
        # Create an execution report using Tangent's underlying engine modules
        ...
        if report["engine_status"] == "COMPLETED":
            # Update Dashboard Metrics Panel
            ...
            # Read the temporarily scrubbed file cache content into memory
            with open(output_temp, 'r', encoding='utf-8') as f:
                self.scrubbed_content_cache = f.read()
            
            # Clean up the local temp cache file instantly
            if os.path.exists(output_temp):
                os.remove(output_temp)

            # Populate Code View Window Area
